...
- **不支持无卡模式开机。** ¥0.1/时 的无卡模式在开放 API 上没有入口。2026-08-24 实测：
  `cpu`、`no_gpu`、`nogpu`、`cpu_only`、`cpu-only`、`none` 一律返回
  `ServerError | 不支持的启动模式`；传空 `payload` 会被接受，但开出来仍然带卡
  （`start_mode: "gpu"`）。需要无卡模式只能用网页控制台。
